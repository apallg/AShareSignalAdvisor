  ...
  <div>
    ...
    <template v-if="stockName">
      ...
      <div class="grid-4 mb-2" v-if="quote.price">
        <MetricCard :value="quote.price" label="最新价" />
        <MetricCard :value="((quote.pct_chg || 0) > 0 ? '+' : '') + (quote.pct_chg || 0) + '%'" label="涨跌幅"
          :color="(quote.pct_chg || 0) > 0 ? 'up' : (quote.pct_chg || 0) < 0 ? 'down' : ''" />
        <MetricCard :value="quote.volume || '--'" label="成交量" />
        <MetricCard :value="quote.high || '--'" label="最高" />
      </div>
...
const { chartRef: klineRef, render: renderChart, dispose: disposeChart } = useChart()
...
async function search() {
  ...
  error.value = ''; stockName.value = ''; loading.value = true
  try {
    const info = await api.get('/stock/' + code.value)
    stockName.value = info.name || code.value
    const [daily, rt, fin, ind] = await Promise.all([
      api.get('/stock/' + code.value + '/daily'),
      api.get('/stock/' + code.value + '/realtime'),
      api.get('/stock/' + code.value + '/financial'),
      api.get('/stock/' + code.value + '/indicators').catch(() => ({ data: {} })),
    ])
    klineData.value = (daily.data || []).map(d => ({ ...d, date: String(d.date || d.trade_date || '').slice(0, 10) }))
    quote.value = rt.data || {}
    financial.value = fin.data || {}
    const id = ind.data || {}
    indicators.value = id.indicators || {}
    signals.value = id.signals || []
    ...
    while (true) {
      ...
      for (const ln of lines) {
        ...
        if (d.startsWith('[AGENT:')) {
          ...
          activeAgent = name; ensureTab(name, name)
          ...
          const msg = d.slice(7)
          if (activeAgent) { agentContents[activeAgent] += '\n⚠️ ' + msg + '\n' }
          else { error.value = msg }
        } else if (d === '[DONE]') {
          statusMsg.value = '分析完成'
          if (agentTabs.length > 0 && !currentTab.value) currentTab.value = agentTabs[0].name
        } else if (activeAgent) {
...
.status-msg { color: #888; font-size: 13px; padding: 6px 0; }
</style>
